...
function _histVideoAddForm(sessionId, startUtc) {
  ...
  return '<div id="hist-video-add-form-' + sessionId + '" style="display:none;margin-top:4px">'
    ...
    + '<button class="btn-export" style="background:var(--action);color:#fff;border-color:var(--action)" onclick="submitHistAddVideo(' + sessionId + ')">Add Video</button>'
...
async function _loadSailsForHistory(sessionId, el) {
  ...
  slots.forEach(slot => {
    ...
    html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">'
      ...
      + '<select id="hist-sail-select-' + slot + '-' + sessionId + '" style="flex:1;background:var(--bg-secondary);color:var(--text-primary);border:1px solid var(--action);border-radius:4px;padding:3px 6px;font-size:.78rem">'
      + '<option value="">— none —</option>' + opts
      + '</select></div>';
  });
  html += '<button class="btn-export" style="background:var(--action);color:#fff;border-color:var(--action);font-size:.78rem" onclick="saveHistSails(' + sessionId + ')">Save Sails</button>';
  html += '</div>';
  el.innerHTML = html;
}
