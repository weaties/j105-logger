...
class AnchorPicker extends HTMLElement {
  ...
  _normalize(raw) {
    ...
    // row so the picker can render a time label, but the moment schema only
    // allows `t_start` on kind=timestamp. Strip it for entity-ref kinds or
    ...
    } else if (raw.kind === 'maneuver' || raw.kind === 'transcript_segment') {
      if (raw.entity_id !== undefined && raw.entity_id !== null) {
        a.entity_id = raw.entity_id;
      }
    }
    // kind === 'session' has no extra fields.
    return a;
  }
