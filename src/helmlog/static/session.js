...
function _videoAddForm() {
  ...
  return '<div id="video-add-form" style="display:none;margin-top:8px">'
    ...
    + '<button class="btn-export" style="background:var(--action);color:#fff;border-color:var(--action)" onclick="submitAddVideo()">Add Video</button>'
...
async function loadSails() {
  ...
  slots.forEach(slot => {
    ...
    html += '<div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">'
      ...
      + '<select id="sail-select-' + slot + '" style="flex:1;background:var(--bg-secondary);color:var(--text-primary);border:1px solid var(--action);border-radius:4px;padding:3px 6px;font-size:.78rem">'
      + '<option value="">\u2014 none \u2014</option>' + opts
      + '</select></div>';
  });
  html += '<button class="btn-export" style="background:var(--action);color:#fff;border-color:var(--action);font-size:.78rem;margin-top:4px" onclick="saveSails()">Save Sails</button>';
...
function renderSharing(data) {
  ...
  for (const c of data.co_ops) {
    if (c.shared) {
      html += '<button class="btn-export" style="background:#0d2818;border:1px solid #16a34a;color:var(--success)"'
        + ' onclick="unshareSession(\'' + esc(c.co_op_id) + '\')">'
        + esc(c.co_op_name) + ' &#10003;</button>';
    } else {
      html += '<button class="btn-export" style="background:#1e293b;border:1px solid #374151;color:var(--text-primary)"'
  ...
  if (data.status === 'candidate') {
    ...
    if (isAdmin) {
      html += '<div style="display:flex;gap:6px">'
        + '<button class="btn-export" style="background:#0d2818;border:1px solid #16a34a;color:var(--success)" onclick="confirmMatch()">Confirm</button>'
    ...
    if (isAdmin) {
      html += '<div style="margin-top:6px">'
        ...
        + ' placeholder="Set shared name" style="background:var(--bg-secondary);border:1px solid #1e3050;border-radius:4px;color:var(--text-primary);padding:4px 8px;font-size:.8rem;width:60%">'
...
function renderPolarHeatmap() {
  ...
  for (const tws of data.tws_bins) {
    ...
    for (const twa of data.twa_bins) {
      ...
      if (!c) {
        html += '<td style="padding:2px 4px;background:var(--bg-secondary);border:1px solid #0a1628"></td>';
        continue;
      }
...
function _showMentionDropdown(el, matches, ctx) {
  ...
  dd.style.cssText = 'position:absolute;z-index:9999;background:#131f35;border:1px solid var(--action);'
